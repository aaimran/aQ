  ...
  subroutine init_domain(D, infile, input_filename)
    ...
    if (stat>0) stop 'error reading namelist problem_list'
     ...
     if (is_master()) then
       ...
       case ('elastic','plastic','anelastic','low-pass','anelastic-Q','anelastic-Q8')
         ! ok
       case default
         invalid_response = .true.
         write(*,*) ':: Error: invalid response option in &problem_list: "' // trim(response_norm) // '"'
         write(*,*) ':: Supported response options are: elastic, plastic, anelastic, low-pass, anelastic-Q, anelastic-Q8'
       end select
     end if
